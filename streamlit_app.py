...
class QdrantIndex:
    ...
    def __init__(self, url: str, api_key: str, index_name: str):
        ...
        self.client = QdrantClient(
            url=url,  # e.g., "https://your-cluster-name.qdrant.tech"
            api_key=api_key,
            timeout=100  # Increased timeout for cloud operations
        )
        ...
        try:
            ...
            if self.index_name not in [c.name for c in collections.collections]:
                self.client.create_collection(
                    ...
                    optimizers_config=qdrant_models.OptimizersConfigDiff(
                        ...
                        memmap_threshold=10000  # Adjust based on your data size
                    )
                )
            ...
            st.error(f"Error initializing Qdrant Cloud collection: {str(e)}")
            raise
        
    def add_documents(self, docs: List[Document], dense_vectors: List[List[float]]):
        """Add documents to Qdrant Cloud"""
        try:
            ...
            batch_size = 50  # Smaller batch size for cloud operations
            for i in range(0, len(points), batch_size):
                batch = points[i:i + batch_size]
                self.client.upsert(
                    collection_name=self.index_name,
                    points=batch,
                    wait=True  # Ensure indexing is complete
                )
                time.sleep(0.2)  # Increased delay for cloud operations
